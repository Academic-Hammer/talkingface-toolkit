...
def build_inference_pipeline(device="cuda"):
    ...
    params = torch.load(
        os.path.join(stargan_pth_dir, stargan_model_name), map_location=device
    )
...
def generate_conversions(
    ...
    for key, (ref, _) in reference_embeddings.items():
        name = pseudo_names[int(key.replace("p", ""))]
        # print(name)
        # start = time.time()
        with torch.no_grad():
        ...
        sf.write(
            "generated_data/"
            + ("" if noise_reduce else "noise_filtered_")
            + f"converted_{name}.mp3",
            recon,
            24000,
        )
...
def plot_melspec(keys, audios, sample_rate=24000, noise_reduce=False):
    ...
    for key in keys:
        ...
        if idy == 2:
            ...
            idy += 1
    # plt.show()
    plt.savefig("generated_data/converted f0.png")
    return f0s
...
audio_np, orig_sr = librosa.load(input_audio_path)
audio = audio_np / np.max(np.abs(audio_np))
wav = librosa.resample(audio, orig_sr=orig_sr, target_sr=24000)
...
sf.write("generated_data/original recording.mp3", wav, 24000)
sf.write("generated_data/noise-filtered recording.mp3", wav_nr, 24000)
...
plt.savefig("generated_data/Original and Noise Filtered Recording.png")
...
plt.savefig("generated_data/Original and Noise Filtered F0.png")
...
target_speakers, converted_samples = generate_conversions(
    wav_nr, emostargan, reference_embeddings, F0_model, vocoder, noise_reduce=False
)
...
tf0s, colours, labels, ftimes = get_F0s_for_plots(wav_nr, vocoder, target_speakers, f0s)
for idx in range(len(f0s)):
    plt.plot(
        ftimes[strt_idx:end_idx],
        tf0s[idx][strt_idx:end_idx],
        color=colours[idx],
...
plt.xlabel("Time (sec)")
plt.ylabel("Pitch (Hz)")
plt.savefig("generated_data/gender_F0.png")
...
for idx in range(len(f0s)):
    plt.plot(
        ...
        color="black" if idx == 0 else list(mcolors.TABLEAU_COLORS.values())[idx],
        linestyle="--" if idx == 0 else "-",
    )
plt.xticks([])
plt.yticks([])
...
plt.ylabel("Pitch (Hz)")
plt.legend()
plt.savefig("generated_data/speaker_F0.png")
...
plt.xticks([])
plt.yticks([])
plt.savefig("generated_data/gender_intensity.png")
...
plt.ylabel("Intensity")
plt.legend()
plt.savefig("generated_data/speaker_intensity.png")
