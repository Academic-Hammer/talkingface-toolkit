...
class FastSpeech2Preprocess:
    def __init__(self, config):
        self.config = config
        self.pre_in_dir = config["path"]["preprocessed_root"]
    ...
    def prepare_align(self):
        with open(os.path.join(self.pre_in_dir, "metadata.csv"), encoding="utf-8") as f:
            for line in tqdm(f):
                ...
                wav_path = os.path.join(self.pre_in_dir, "wavs", "{}.wav".format(base_name))
                if os.path.exists(wav_path):
                    os.makedirs(os.path.join(self.in_dir, self.speaker), exist_ok=True)
                    wav, _ = librosa.load(wav_path)
                    wav = wav / max(abs(wav)) * self.max_wav_value
                    wavfile.write(
                        os.path.join(self.in_dir, self.speaker, "{}.wav".format(base_name)),
                        sampling_rate,
                        wav.astype(np.int16),
                    )
                    with open(
                        os.path.join(self.in_dir, self.speaker, "{}.lab".format(base_name)),
                        "w",
                    ) as f1:
                        f1.write(text)
