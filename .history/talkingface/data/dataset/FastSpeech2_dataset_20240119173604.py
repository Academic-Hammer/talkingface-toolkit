...
class FastSpeech2Dataset(Dataset):
    def __init__(
        self, filename, config, config, sort=False, drop_last=False
    ):
...
if __name__ == "__main__":
    ...
    train_dataset = Dataset(
        "train.txt", config, config, sort=True, drop_last=True
    )
    val_dataset = Dataset(
        "val.txt", config, config, sort=False, drop_last=False
    )
    train_loader = DataLoader(
        train_dataset,
        batch_size=config["optimizer"]["batch_size"] * 4,
        shuffle=True,
