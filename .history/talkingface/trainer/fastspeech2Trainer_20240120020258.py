...
class FastSpeech2Trainer(Trainer):
    def __init__(self, config, model):
        ...
        self.model, self.optimizer = self.get_model(config, self.device, train=True)
    ...
    def get_vocoder(self, config, device):
        ...
        if name == "MelGAN":
            ...
            with open("talkingface/utils/fastspeech2_transformerblock/config.json", "r") as f:
                config = json.load(f)
    ...
    def _train_epoch(self,train_data,config,epoch_idx, loss_func=None, show_progress=False):
        self.model.train()
        total_loss_dict = {}
        step = 0
        ...
        for batch_idx, batch in enumerate(iter_data):
            ...
            total_loss = losses[0] / config["optimizer"]["grad_acc_step"]
            total_loss.backward()

            if (step + 1) % config["optimizer"]["grad_acc_step"] == 0:
                nn.utils.clip_grad_norm_(self.model.parameters(
                ), config["optimizer"]["grad_clip_thresh"])
    ...
    def _valid_epoch(self, valid_data, loss_func=None, show_progress=False):
        ...
        iter_data = tqdm(valid_data, total=len(valid_data),
                         ncols=None, desc="Validating") if show_progress else valid_data
        ...
        with torch.no_grad():  # 在验证时不计算梯度
            for batch_idx, batch in enumerate(iter_data):
                ...
                for i, l in enumerate(losses):
                    key = f"loss_{i}"
                    total_loss_dict[key] = total_loss_dict.get(key, 0) + l.item()
