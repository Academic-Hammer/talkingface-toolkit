...
class FastSpeech2Trainer(Trainer):
    def __init__(self, config, model):
        ...
        self.loss = FastSpeech2Loss(config["preprocess_config"], config["config"]).to(self.device)
    ...
    def get_model(self,config, device, train=False):
        ...
        if os.path.isdir(ckpt_dir):
            ...
            # 按文件修改时间排序并获取最新的文件
            if ckpt_files:
                latest_ckpt = max(ckpt_files, key=lambda x: os.path.getmtime(os.path.join(ckpt_dir, x)))
...
class ScheduledOptim:
    ...
    def __init__(self, model, train_config, config, current_step):
