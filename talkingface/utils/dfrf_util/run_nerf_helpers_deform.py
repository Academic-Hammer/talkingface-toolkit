...
def mse2psnr(x): return -10. * torch.log(x.to(device)).to(device) / torch.log(torch.Tensor([10.])).to(device)
...
def get_rays(H, W, focal, c2w, cx=None, cy=None):
    ...
    if cy is None:
        cy = H*.5
    dirs = torch.stack(
        [(i-cx)/focal, -(j-cy)/focal, -torch.ones_like(i)], -1).to("cuda:0")
...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    # Invert CDF
    u = u.contiguous()
    inds = torch.searchsorted(cdf.to(device), u.to(device), right=True)
    ...
    denom = torch.where(denom < 1e-5, torch.ones_like(denom), denom).to(device)
    t = (u.to(device)-cdf_g.to(device)[..., 0])/denom
...
def make_indices(pts, attention_poses, intrinsic, H, W):
    pts = pts.to(device)
    attention_poses = attention_poses.to(device)
    intrinsic = intrinsic.to(device)
    hom_points = torch.cat((pts, torch.broadcast_to(torch.tensor([1.0]).to(device), pts.shape[:-1] + (1,))), -1)
    ...
    final = (torch.Tensor([0., W]).to(device) - final) * torch.Tensor([-1., 1.]).to(device)
    #final = torch.round(final)
    final = torch.maximum(torch.minimum(final, torch.Tensor([H-1.,W-1.]).to(device)), torch.Tensor([0,0]).to(device))
    #final = final.int()
    return final
...
class Position_warp(nn.Module):
    ...
    def forward(self, query_pts, aud, reference_feature):
        ...
        output = torch.maximum(torch.minimum(output, torch.Tensor([10,10]).to(device)), torch.Tensor([-10,-10]).to(device))
        return output
