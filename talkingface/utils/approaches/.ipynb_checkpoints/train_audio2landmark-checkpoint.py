...
from talkingface.utils.src.dataset.audio2landmark.audio2landmark_dataset import Audio2landmark_Dataset
from talkingface.utils.src.models.model_audio2landmark import *
from talkingface.utils.util.utils import get_n_params
import numpy as np
import pickle
...
class Audio2landmark_model():
    ...
    def __init__(self, opt_parser, jpg_shape=None):
        ...
        config = opt_parser
        self.std_face_id = np.loadtxt('talkingface/utils/src/dataset/utils/STD_FACE_LANDMARKS.txt')
        ...
        self.eval_data = Audio2landmark_Dataset(dump_dir='dataset/examples/dump',
                                                dump_name='random',
                                                status='val',
        ...
        ckpt = torch.load(config['load_a2l_C_name'])
        self.C.load_state_dict(ckpt['model_g_face_id'])
        # self.C.load_state_dict(ckpt['C'])
        print('======== LOAD PRETRAINED FACE ID MODEL {} ========='.format(config['load_a2l_C_name']))
        self.C.to(device)

        self.t_shape_idx = (27, 28, 29, 30, 33, 36, 39, 42, 45)
        self.anchor_t_shape = np.loadtxt('talkingface/utils/src/dataset/utils/STD_FACE_LANDMARKS.txt')
        self.anchor_t_shape = self.anchor_t_shape[self.t_shape_idx, :]

        with open(os.path.join('dataset/examples', 'dump', 'emb.pickle'), 'rb') as fp:
    ...
    #
        ...
        fl_dis_pred = torch.tensor(fl_dis_pred).to(device) * self.config['amp_pos']
    ...
    def __calib_baseline_pred_fls_old_(self, baseline_pred_fls, residual_face_id, aus):
        ...
        baseline_pred_fls[:, 48 * 3::3] *= self.config['amp_lip_x']  # mouth x
        baseline_pred_fls[:, 48 * 3 + 1::3] *= self.config['amp_lip_y']  # mouth y
    ...
    def __train_pass__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            keys = self.config['reuse_train_emb_list']
            if(len(keys) == 0):
                keys = ['audio_embed']
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                if(no_y_rotation):
                    ...
                    from talkingface.utils.icp import icp
                ...
                np.savetxt(os.path.join(self.config['output_folder'], filename), fake_fls_np, fmt='%.6f')

                # ''' Visualize result in landmarks '''
                if(vis_fls):
                    from talkingface.utils.vis import Vis
    ...
    def __close_face_lip__(self, fl):
        ...
        from talkingface.utils.geo_math import area_of_polygon
    ...
    def __solve_inverse_lip2__(self, fl_dis_pred_pos_numpy):
        for j in range(fl_dis_pred_pos_numpy.shape[0]):
            ...
            from talkingface.utils.geo_math import area_of_signed_polygon
