...
#         writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256 * 3, 256))
        writer_ref_in = cv2.VideoWriter('out_ref_in.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
        writer_g_out = cv2.VideoWriter('out_g_out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
        writer_fls_in = cv2.VideoWriter('out_fls_in.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
...
#             prefix, filename[:-4]))
        ...
        os.system('ffmpeg -loglevel error -y -i out_g_out.mp4 -i {} -pix_fmt yuv420p -strict -2 examples/{}_{}_g_out.mp4'.format(
        'examples/'+filename[9:-16]+'.wav',
        prefix, filename[:-4]))
