...
from talkingface.utils.src.models.model_image_translation import ResUnetGenerator, VGGLoss
import torch
...
import cv2
import os, glob
from talkingface.utils.src.dataset.image_translation.image_translation_dataset import vis_landmark_on_img, vis_landmark_on_img98, vis_landmark_on_img74
...
class Image_translation_block():
    ...
    def __init__(self, opt_parser, single_test=False):
        ...
        self.config = opt_parser
        config = opt_parser

        # model
        if(config['add_audio_in']):
        ...
        if (config['load_G_name'] != ''):
            ckpt = torch.load(config['load_G_name'])
            try:
        ...
        if(not single_test):
            # dataset
            if(config['use_vox_dataset'] == 'raw'):
                if(config['comb_fan_awing']):
                    from talkingface.utils.src.dataset.image_translation.image_translation_dataset import \
                        image_translation_raw74_dataset as image_translation_dataset
                elif(config['add_audio_in']):
                    from talkingface.utils.src.dataset.image_translation.image_translation_dataset import image_translation_raw98_with_audio_dataset as \
                        image_translation_dataset
                else:
                    from talkingface.utils.src.dataset.image_translation.image_translation_dataset import image_translation_raw98_dataset as \
                    image_translation_dataset
            else:
                from talkingface.utils.src.dataset.image_translation.image_translation_dataset import image_translation_preprocessed98_dataset as \
                    image_translation_dataset

            self.dataset = image_translation_dataset(num_frames=config['num_frames'])
            ...
            # optimizer
            self.optimizer = torch.optim.Adam(self.G.parameters(), lr=config['lr'], betas=(0.5, 0.999))

            # writer
            if(config['write']):
                self.writer = SummaryWriter(log_dir=os.path.join(config['log_dir'], config['name']))
                self.count = 0
    ...
    def __train_pass__(self, epoch, is_training=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            if (i % self.config['jpg_freq'] == 0):
                ...
                try:
                    os.makedirs(os.path.join(self.config['jpg_dir'], self.config['name']))
                except:
                    pass
                cv2.imwrite(os.path.join(self.config['jpg_dir'], self.config['name'], 'e{:03d}_b{:04d}.jpg'.format(epoch, i)), vis * 255.0)
            # save ckpt
            if (i % self.config['ckpt_last_freq'] == 0):
                self.__save_model__('last', epoch)

            print("Epoch {}, Batch {}/{}, loss {:.4f}, l1 {:.4f}, vggloss {:.4f}, styleloss {:.4f} time {:.4f}".format(
                epoch, i, len(self.dataset) // self.config['batch_size'],
    ...
    def __save_model__(self, save_type, epoch):
        try:
            os.makedirs(os.path.join(self.config['ckpt_dir'], self.config['name']))
        except:
            pass
        if (self.config['write']):
            torch.save({
            ...
            'opt': self.optimizer,
            'epoch': epoch
        }, os.path.join(self.config['ckpt_dir'], self.config['name'], 'ckpt_{}.pth'.format(save_type)))
    ...
    def test(self):
        if (self.config['use_vox_dataset'] == 'raw'):
            if(self.config['add_audio_in']):
                from talkingface.utils.src.dataset.image_translation.image_translation_dataset import \
                    image_translation_raw98_with_audio_test_dataset as image_translation_test_dataset
            else:
                from talkingface.utils.src.dataset.image_translation.image_translation_dataset import image_translation_raw98_test_dataset as image_translation_test_dataset
        else:
            from talkingface.utils.src.dataset.image_translation.image_translation_dataset import image_translation_preprocessed98_test_dataset as image_translation_test_dataset
        self.dataset = image_translation_test_dataset(num_frames=self.config['num_frames'])
        self.dataloader = torch.utils.data.DataLoader(self.dataset,
                                                      batch_size=1,
                                                      shuffle=True,
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False):
        ...
        if(jpg is None):
            jpg = glob.glob1(self.config['single_test'], '*.jpg')[0]
            jpg = cv2.imread(os.path.join(self.config['single_test'], jpg))

        if(fls is None):
            fls = glob.glob1(self.config['single_test'], '*.txt')[0]
            fls = np.loadtxt(os.path.join(self.config['single_test'], fls))
            fls = fls * 95
            fls[:, 0::3] += 130
            fls[:, 1::3] += 80

#         writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256 * 3, 256))
        writer_ref_in = cv2.VideoWriter('dataset/examples/out_ref_in.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
        writer_g_out = cv2.VideoWriter('dataset/examples/out_g_out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
        writer_fls_in = cv2.VideoWriter('dataset/examples/out_fls_in.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
...
#             ref_in_frame = ref_in[i] * 255.0
#             g_out_frame = g_out[i] * 255.0
#             fls_in_frame = fls_in[i] * 255.0

#             writer_ref_in.write(ref_in_frame.astype(np.uint8))
#             writer_g_out.write(g_out_frame.astype(np.uint8))
#             writer_fls_in.write(fls_in_frame.astype(np.uint8))
...
#         writer.release()
#         writer_ref_in.release()
        writer_g_out.release()
#         writer_fls_in.release()
...
#         os.system('ffmpeg -loglevel error -y -i dataset/examples/out_g_out.mp4 -i {} -pix_fmt yuv420p -strict -2 dataset/examples/{}_{}_g_out.mp4'.format(
#         'dataset/examples/'+filename[9:-16]+'.wav',
#         prefix, filename[:-4]))
        os.system('ffmpeg -loglevel error -y -i dataset/examples/out_g_out.mp4 -i {} -pix_fmt yuv420p -strict -2 dataset/examples/generate.mp4'.format(
        'dataset/examples/'+filename[9:-16]+'.wav'))
