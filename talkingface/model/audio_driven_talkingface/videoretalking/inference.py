...
def main():    
    ...
    # 获取当前脚本所在目录的上一级目录的上一级目录的上一级目录
    grandparent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..','..', '..', '..'))

    # 组合父目录下的'results/temp'路径
    temp_dir = os.path.join(grandparent_dir, 'results', 'temp', 'temp')

    # 创建目录，如果已经存在则不引发错误
    os.makedirs(temp_dir, exist_ok=True)

    enhancer = FaceEnhancement(base_dir='../../../../checkpoints/video-retalking', size=512, model='GPEN-BFR-512', use_sr=False, \
                               sr_model='rrdb_realesrnet_psnr', channel_multiplier=2, narrow=1, device=device)
    restorer = GFPGANer(model_path='../../../../checkpoints/video-retalking/GFPGANv1.3.pth', upscale=1, arch='clean', \
    ...
    croper = Croper('../../../../checkpoints/video-retalking/shape_predictor_68_face_landmarks.dat')
    ...
    if not os.path.isfile('../../../../results/temp/'+base_name+'_landmarks.txt') or args.re_preprocess:
        print('[Step 1] Landmarks Extraction in Video.')
        kp_extractor = KeypointExtractor()
        lm = kp_extractor.extract_keypoint(frames_pil, '../../../../results/temp/'+base_name+'_landmarks.txt')
    else:
        print('[Step 1] Using saved landmarks.')
        lm = np.loadtxt('../../../../results/temp/'+base_name+'_landmarks.txt').astype(np.float32)
        lm = lm.reshape([len(full_frames), -1, 2])
       
    if not os.path.isfile('../../../../results/temp/'+base_name+'_coeffs.npy') or args.exp_img is not None or args.re_preprocess:
        net_recon = load_face3d_net(args.face3d_net_path, device)
        lm3d_std = load_lm3d('../../../../checkpoints/video-retalking/BFM')
        ...
        np.save('../../../../results/temp/'+base_name+'_coeffs.npy', semantic_npy)
    else:
        print('[Step 2] Using saved coeffs.')
        semantic_npy = np.load('../../../../results/temp/'+base_name+'_coeffs.npy').astype(np.float32)
    ...
    if args.exp_img is not None and ('.png' in args.exp_img or '.jpg' in args.exp_img):
        ...
        W, H = exp_pil.size
        kp_extractor = KeypointExtractor()
        lm_exp = kp_extractor.extract_keypoint([exp_pil], '../../../../results/temp/'+base_name+'_temp.txt')[0]
        if np.mean(lm_exp) == -1:
        ...
        expression = torch.tensor(loadmat('../../../../checkpoints/video-retalking/expression.mat')['expression_mouth'])[0]
    else:
        print('using expression center')
        expression = torch.tensor(loadmat('../../../../checkpoints/video-retalking/expression.mat')['expression_center'])[0]

    # load DNet, model(LNet and ENet)
    D_Net, model = load_model(args, device)

    if not os.path.isfile('../../../../results/temp/'+base_name+'_stablized.npy') or args.re_preprocess:
        imgs = []
        ...
        np.save('../../../../results/temp/'+base_name+'_stablized.npy',imgs)
        del D_Net
    else:
        print('[Step 3] Using saved stabilized video.')
        imgs = np.load('../../../../results/temp/'+base_name+'_stablized.npy')
    torch.cuda.empty_cache()

    if not args.audio.endswith('.wav'):
        command = 'ffmpeg -loglevel error -y -i {} -strict -2 {}'.format(args.audio, 'temp/{}/temp.wav'.format(args.tmp_dir))
        subprocess.call(command, shell=True)
        args.audio = '../../../../results/temp/{}/temp.wav'.format(args.tmp_dir)
    ...
    out = cv2.VideoWriter('../../../../results/temp/{}/result.mp4'.format(args.tmp_dir), cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_w, frame_h))
    ...
    command = 'ffmpeg -loglevel error -y -i {} -i {} -strict -2 -q:v 1 {}'.format(args.audio, '../../../../results/temp/{}/result.mp4'.format(args.tmp_dir), args.outfile)
...
def datagen(frames, mels, full_frames, frames_pil, cox):
    ...
    lms = kp_extractor.extract_keypoint(fr_pil, '../../../../results/temp/'+base_name+'x12_landmarks.txt')
