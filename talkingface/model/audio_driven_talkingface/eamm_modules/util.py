...
def draw_heatmap(landmark, width, height):
    ...
    for i in range(batch):
        for pts_idx in range(number):
            ...
            heatmap[
                i, pts_idx, int(landmark[i, pts_idx, 1]), int(landmark[i, pts_idx, 0])
            ] = 1
            if heatmap[i, pts_idx].sum() == 1:
                heatmap[i, pts_idx] = cv2.GaussianBlur(
                    heatmap[i, pts_idx], ksize=(3, 3), sigmaX=1, sigmaY=1
                )
...
class NA_net(nn.Module):
    def __init__(self):
        ...
        self.decon = nn.Sequential(
            nn.ConvTranspose2d(
                1, 16, kernel_size=(2, 3), stride=2, padding=(2, 1), bias=True
            ),  # 16,16
            nn.BatchNorm2d(16),
            nn.ReLU(True),
            nn.ConvTranspose2d(
                16, 32, kernel_size=4, stride=2, padding=1, bias=True
            ),  # 8,8
            nn.BatchNorm2d(32),
            nn.ReLU(True),
            nn.ConvTranspose2d(
                32, 32 + 3, kernel_size=4, stride=2, padding=1, bias=True
            ),  # 16,16
        )
...
class AT_net(nn.Module):
    def __init__(self):
        ...
        self.decon = nn.Sequential(
            nn.ConvTranspose2d(
                256, 256, kernel_size=6, stride=2, padding=1, bias=True
            ),  # 4,4
            nn.BatchNorm2d(256),
            nn.ReLU(True),
            nn.ConvTranspose2d(
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(
                128, 32 + 3, kernel_size=4, stride=2, padding=1, bias=True
            ),  # 64,64
    ...
    def forward(self, example_image, audio, pose, jaco_net):
        hidden = (
            torch.autograd.Variable(
                torch.zeros(3, audio.size(0), 256).to(example_image.device)
            ),
            torch.autograd.Variable(
                torch.zeros(3, audio.size(0), 256).to(example_image.device)
            ),
...
class TF_net(nn.Module):
    ...
    def forward(self, example_image, audio, pose, jaco_net, emo_features):
        ...
        for step_t in range(audio.size(1)):
            ...
            pose_f = self.pose_encoder(pose[:, step_t])  # 256
            features = torch.cat(
                [image_feature, current_feature, pose_f, emo_features[step_t]], 1
            )
            lstm_input.append(features)
        lstm_input = torch.stack(lstm_input, dim=1)
...
class AF2F(nn.Module):
    def __init__(self):
        ...
        self.decon = nn.Sequential(
            nn.ConvTranspose2d(
                384, 256, kernel_size=6, stride=2, padding=1, bias=True
            ),  # 4,4
            nn.BatchNorm2d(256),
            nn.ReLU(True),
            nn.ConvTranspose2d(
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(
                64, 32 + 3, kernel_size=4, stride=2, padding=1, bias=True
            ),  # 64,64
        )
...
class A2I(nn.Module):
    def __init__(self):
        ...
        self.decon = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(
                32, 2, kernel_size=4, stride=2, padding=1, bias=True
            ),  # 64,64
            nn.ReLU(),
        )
...
class Emotion_map(nn.Module):
    ...
    def __init__(
        ...
        self.fc_all = nn.Sequential(nn.Linear(1024, 2048), nn.ReLU(True))
        ...
        self.classify = Classify()
        self.kp = nn.Conv2d(
            in_channels=35, out_channels=10, kernel_size=(7, 7), padding=0
        )
        self.jacobian = nn.Conv2d(
            in_channels=35, out_channels=4 * 10, kernel_size=(7, 7), padding=0
        )
        self.jacobian.weight.data.zero_()
        self.jacobian.bias.data.copy_(
            torch.tensor([1, 0, 0, 1] * 10, dtype=torch.float)
        )
        self.temperature = 0.1

        self.kp_4 = nn.Conv2d(
            in_channels=35, out_channels=4, kernel_size=(7, 7), padding=0
        )
        self.jacobian_4 = nn.Conv2d(
            in_channels=35, out_channels=4 * 4, kernel_size=(7, 7), padding=0
        )
        self.jacobian_4.weight.data.zero_()
        self.jacobian_4.bias.data.copy_(
    ...
    def map_4(self, x, value, jacobian):  # torch.Size([4, 3, H, W])
        ...
        if self.jacobian is not None:
            ...
            jacobian_map = jacobian_map.reshape(
                final_shape[0], 4, 4, final_shape[2], final_shape[3]
            )
    ...
    def forward(self, x, value, jacobian):  # torch.Size([4, 3, H, W])
        ...
        if self.jacobian is not None:
            ...
            jacobian_map = jacobian_map.reshape(
                final_shape[0], 10, 4, final_shape[2], final_shape[3]
            )
            heatmap = heatmap.unsqueeze(2)
