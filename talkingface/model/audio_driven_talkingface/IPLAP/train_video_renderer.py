...
parser.add_argument('--sketch_root', help='root path for sketches',
                    default='dataset/lrs2/lrs2_preprocessed/lrs2_sketch')
parser.add_argument('--face_img_root', help='root path for face frame images',
                    default='dataset/lrs2/lrs2_preprocessed/lrs2_face')
parser.add_argument('--audio_root', help='root path for audio mel',
                    default='dataset/lrs2/lrs2_preprocessed/lrs2_audio')
...
class Dataset(object):
    def get_vid_name_list(self, split):
        filelist = []
        with open('dataset/{}/filelists/{}.txt'.format(filelist_name, split)) as f:
            for line in f:
                line = line.strip()
...
def save_sample_images_gen(T_frame_sketch, ref_N_frame_img, wrapped_ref, generated_img, gt, global_step,
                           checkpoint_dir):
    ...
    ref_N_frame_img = (ref_N_frame_img.cpu().numpy().transpose(0, 1, 2, 4, 5, 3) * 255.).astype(
        np.uint8)  # ref: (B,T,ref_N,H,W,3)
    ...
    T_frame_sketch = (T_frame_sketch[:, 2].unsqueeze(1).cpu().numpy().transpose(0, 1, 3, 4, 2) * 255.).astype(
        np.uint8)  # (B,T,H,W,3)

    folder = join(checkpoint_dir, "samples_step{:09d}".format(global_step))
    if not os.path.exists(folder):
        os.mkdir(folder)
    collage = np.concatenate((T_frame_sketch, *[ref_N_frame_img[:, :, i] for i in range(ref_N_frame_img.shape[2])],
                              wrapped_ref, fake_image, gt),
                             axis=-2)
...
def evaluate(model, val_data_loader):
        ...
        for step, (T_frame_img, T_frame_sketch, ref_N_frame_img, ref_N_frame_sketch, T_mels) in prog_bar:
            ...
            T_frame_img, T_frame_sketch, ref_N_frame_img, ref_N_frame_sketch, T_mels = \
                T_frame_img.cuda(non_blocking=True), T_frame_sketch.cuda(non_blocking=True), \
                    ref_N_frame_img.cuda(non_blocking=True), ref_N_frame_sketch.cuda(non_blocking=True), T_mels.cuda(
                    non_blocking=True)
            ...
            ssims.append(ssim)
            fids.append(fid)
        save_sample_images_gen(T_frame_sketch, ref_N_frame_img, wrapped_ref, generated_img, gt, global_step,
                               checkpoint_dir)
        #                         (B,T,3,H,W)  (B,ref_N,3,H,W)  (B*T,3,H,W) (B*T,3,H,W)(B*T,3,H,W)
    ...
    print('eval_warp_loss :', eval_warp_loss / count, 'eval_gen_loss', eval_gen_loss / count, 'global_step:',
          global_step)
    ...
    while global_epoch < 9999999999:
        ...
        for step, (T_frame_img, T_frame_sketch, ref_N_frame_img, ref_N_frame_sketch, T_mels) in prog_bar:
            ...
            prog_bar.set_description('epoch: %d step: %d running_warp_loss: %.4f running_gen_loss: %.4f' \
                                     % (global_epoch, global_step, running_warp_loss / (step + 1),
