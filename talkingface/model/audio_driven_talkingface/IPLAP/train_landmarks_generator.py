...
parser.add_argument('--pre_audio_root', default='dataset/lrs2/lrs2_preprocessed/lrs2_audio',
                    help='root path for preprocessed  audio')
parser.add_argument('--landmarks_root', default='dataset/lrs2/lrs2_preprocessed/lrs2_landmarks',
...
full_face_sequence = [*list(range(0, 4)), *list(range(21, 25)), *list(range(25, 91)), *list(range(4, 21)),
                      *list(range(91, 131))]
...
def evaluate(model, val_data_loader):
    ...
    for epoch in tqdm(range(eval_epochs), total=eval_epochs):
        ...
        for step, (T_mels, T_pose, T_content, Nl_pose, Nl_content) in prog_bar:
            model.eval()
            T_mels, T_pose, T_content, Nl_pose, Nl_content = T_mels.cuda(non_blocking=True), T_pose.cuda(
                non_blocking=True), T_content.cuda(non_blocking=True), \
...
if __name__ == '__main__':
    ...
    if finetune_path is not None:  ###fine tuning
    ...
    while global_epoch < 9999999999:
        ...
        for step, (T_mels, T_pose, T_content, Nl_pose, Nl_content) in prog_bar:
            ...
            optimizer.zero_grad()
            predict_content = model(T_mels, T_pose, Nl_pose, Nl_content)  # (B*T,2,57)
            T_content = torch.cat([T_content[i] for i in range(T_content.size(0))],
                                  dim=0)  # (B*T,2,57):ground truth lip and jaw landmarks
