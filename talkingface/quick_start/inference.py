...
import torch
import os
import talkingface.quick_start.inference_util as inference_util
import talkingface.model.image_driven_talkingface.styleheat.utils.video_util as video_util
from talkingface.utils.SadTalker.src.utils.init_path import init_path
from talkingface.config.config import Config
from talkingface.config.path import PRETRAINED_MODELS_PATH
from talkingface.model.image_driven_talkingface.styleheat.styleheat.styleheat import StyleHEAT


def reenactment(args,generator, data):
    """
    :param generator:
    :param data: {
    ...
    for _i in tqdm.tqdm(range(num_batch),'generator.forward'):
...
def audio_reenactment(args,generator, data, audio_path):
    # import TODO
    import sys
    import os
    sys.path.append(os.path.abspath('./talkingface/utils/SadTalker/'))
    import random
    from pydub import AudioSegment
    from talkingface.utils.SadTalker.src.test_audio2coeff import Audio2Coeff
    from talkingface.utils.SadTalker.src.generate_batch import get_data
    import scipy.io as scio
    import warnings
    warnings.filterwarnings("ignore")

    current_root_path = os.path.abspath('./talkingface/utils/SadTalker/')
    ...
    sadtalker_paths = init_path(os.path.join(current_root_path,"checkpoints"), os.path.join(current_root_path,"src", "config"))
    ...
    audio_batch = get_data(first_coeff_path, audio_path, device,None)
    ...
    reenactment(args,generator, data)
    # import pdb; pdb.set_trace()
...
def attribute_edit(args,generator, data):
...
def intuitive_edit(args,generator, data):
    source_image = data['source_align'].unsqueeze(0).cuda()
...
def parse_args():
    ...
    parser.add_argument('--config', default='./talkingface/properties/model/StyleHeat/inference.yaml')
...
def main(args):
    ...
    for _ in tqdm.tqdm(range(len(dataset))):
        ...
        if args.intuitive_edit:
            intuitive_edit(args,generator, data)
        elif args.attribute_edit:
            attribute_edit(args,generator, data)
        elif args.audio_path is not None:
            audio_reenactment(args, generator, data, args.audio_path)
        else:
            reenactment(args,generator, data)
