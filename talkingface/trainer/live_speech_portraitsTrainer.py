...
class live_speech_portraitsTrainer(AbstractTrainer):
...
#        self.write_video_with_audio(tmp_audio_path, final_path, 'pred_')
        ...
        for j in tqdm(range(nframe), position=0, desc='writing video'):
            img = cv2.imread(join(save_root, 'pred_' + str(j + 1) + '.jpg'))
            out.write(img)
        out.release()
        cmd = 'ffmpeg -i "' + video_tmp_path + '" -i "' + tmp_audio_path + '" -codec copy -shortest "' + final_path + '"'
        subprocess.call(cmd, shell=True)
        os.remove(video_tmp_path)  # remove the template video

        feature_maps_path = join(save_root, audio_name + '_feature_maps.avi')
#        self.write_video_with_audio(tmp_audio_path, feature_maps_path, 'input_')
        fps, fourcc = 60, cv2.VideoWriter_fourcc(*'DIVX')
        video_tmp_path = join(save_root, 'tmp.avi')
        out = cv2.VideoWriter(video_tmp_path, fourcc, fps, (Renderopt.loadSize, Renderopt.loadSize))
        for j in tqdm(range(nframe), position=0, desc='writing video'):
            img = cv2.imread(join(save_root, 'input_' + str(j + 1) + '.jpg'))
            out.write(img)
        out.release()
        cmd = 'ffmpeg -i "' + video_tmp_path + '" -i "' + tmp_audio_path + '" -codec copy -shortest "' + feature_maps_path + '"'
        subprocess.call(cmd, shell=True)
        os.remove(video_tmp_path)  # remove the template video
