...
class FastSpeech2Trainer(Trainer):
    ...
    def get_vocoder(self, config, device):
        ...
        if name == "MelGAN":
            ...
            with open("checkpoints/HiFi-GAN.json", "r") as f:
                config = json.load(f)
    ...
    def evaluate(self, load_best_model=True, model_file=None):
        ...
        dataset = FastSpeech2Dataset(
            self.config, self.config['test_filelist'], sort=False, drop_last=False
        )
        ...
        message = "Total Loss: {:.4f}, Mel Loss: {:.4f}, Mel PostNet Loss: {:.4f}, Pitch Loss: {:.4f}, Energy Loss: {:.4f}, Duration Loss: {:.4f}".format(
