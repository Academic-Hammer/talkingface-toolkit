...
def get_others(root, video_name, data_type):
    ...
    if data_type == 'keypoint':
        if os.path.isfile(os.path.join(root, 'train', video_name+ '.txt')):
            file_path = os.path.join(root, 'train', video_name+ '.txt')
        else:
            file_path = os.path.join(root, 'test', video_name+ '.txt')
...
def convert_kp(file_path):
    ...
    kps = np.loadtxt(file_path,dtype=np.float32)
...
def prepare_data(path, keypoint_path, coeff_3dmm_path, out, n_worker, sizes, chunksize, img_format):
    ...
    for size in sizes:
        ...
        with lmdb.open(lmdb_path, map_size=4* 1024 ** 3, readahead=False) as env:
