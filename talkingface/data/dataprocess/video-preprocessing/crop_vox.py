...
def save_bbox_list(video_path, bbox_list, _args):
    f = open(os.path.join(_args.bbox_folder, os.path.basename(video_path)[:-4] + '.txt'), 'w')
...
def estimate_bbox(person_id, video_id, video_path, fa, args):
    ...
    save_bbox_list(video_path, bbox_list,args)
...
def download(video_id, args):
    ...
    subprocess.call([args.youtube, '-f', "''best/mp4''", '--write-auto-sub', '--write-sub',
                     ...
                     video_path],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    return video_path
...
def split_in_utterance(person_id, video_id, args):
    ...
    for i, utterance in enumerate(utterances):
        ...
        subprocess.call(['ffmpeg', '-c:v','h264_cuvid','-y', '-i', video_path, '-qscale:v',
                         '5', '-r', '25', '-threads', '1', '-ss', str(first_frame), '-to', str(last_frame),
                         '-strict', '-2', '-vf','yadif', chunk_name],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    return chunk_names
...
def run(params):
    ...
    chunks_data = []
    # from tqdm import tqdm
    # for video_id in tqdm(os.listdir(video_folder)):
    from tqdm import tqdm
    from colorama import Fore
    vids =  os.listdir(video_folder)
    for video_id in tqdm(vids,bar_format='{l_bar}%s{bar}%s{r_bar}' % (Fore.BLACK, Fore.RESET),desc=""):
    # for video_id in vids:
        ...
        try:
            ...
            if args.estimate_bbox:
                ...
                # for chunk in tqdm(glob.glob(path),bar_format='{l_bar}%s{bar}%s{r_bar}' % (Fore.CYAN, Fore.RESET)):
