...
STD_FACE_LANDMARK_FILE_DIR = 'talkingface-toolkit-main/talkingface/utils/dataset_utils/utils/STD_FACE_LANDMARKS.txt'


#class Audio2landmark_Dataset(data.Dataset):
class Audio2landmark_contentDataset(data.Dataset):

    def __init__(self,dump_dir='dataset/dump', dump_name='autovc_retrain_mel', num_window_frames=18, num_window_step=1, status='train'):
        self.dump_dir = 'dataset/dump'
#         print(dump_dir,type(dump_dir))
        dump_name = 'autovc_retrain_mel'
        self.num_window_frames = num_window_frames
        self.num_window_step = num_window_step
        print(dump_name,type(dump_name),status,type(status))
        ...
        au_mean_std = np.loadtxt('talkingface/utils/dataset_utils/utils/MEAN_STD_AUTOVC_RETRAIN_MEL_AU.txt')
...
def norm_output_fls_rot(fl_data_i, anchor_t_shape=None):
    ...
    if(anchor_t_shape is None):
        anchor_t_shape = np.loadtxt(
            r'talkingface/utils/dataset_utils/utils/ANCHOR_T_SHAPE_{}.txt'.format(len(t_shape_idx)))
...
class Speaker_aware_branch_Dataset(data.Dataset):
    ...
    def __init__(self, dump_dir, dump_name, num_window_frames, num_window_step, status, use_11spk_only=False, noautovc=''):
        ...
        au_mean_std = np.loadtxt('talkingface/utils/dataset_utils/utils/MEAN_STD_AUTOVC_RETRAIN_MEL_AU.txt') # np.mean(self.au_data[0][0]), np.std(self.au_data[0][0])
        au_mean, au_std = au_mean_std[0:au_mean_std.shape[0]//2], au_mean_std[au_mean_std.shape[0]//2:]
