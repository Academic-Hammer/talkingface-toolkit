...
from voice_conversion.Utils.constants import MEL_PARAMS, EMO_DICT, EMO_DB_DICT
...
class MelDataset(torch.utils.data.Dataset):
    def __init__(self, data_list, sr=24000, validation=False, domain=None):
        _data_list = [l[:-1].split("|") for l in data_list]
        self.data_list = [
            (path, int(label.replace("p", ""))) for path, label in _data_list
        ]
    ...
    def __getitem__(self, idx):
        ...
        #     "label": label,
        #     "sp_label": sp_label,
        #     "ref_mel_tensor": ref_mel_tensor,
        #     "ref2_mel_tensor": ref2_mel_tensor,
        #     "ref_label": ref_label,
        # }
...
class Collater(object):
    ...
    def __call__(self, batch):
        ...
        for bid, (mel, label, sp_label, ref_mel, ref2_mel, ref_label) in enumerate(
            batch
        ):
            mel_size = mel.size(1)
        ...
        mels, ref_mels, ref2_mels = (
            mels.unsqueeze(1),
            ref_mels.unsqueeze(1),
            ref2_mels.unsqueeze(1),
        )
