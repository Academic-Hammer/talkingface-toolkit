...
class MappingNetwork(nn.Module):
    def __init__(
        self, latent_dim=16, style_dim=48, num_speaker_domains=2, hidden_dim=384
    ):
        super().__init__()
        layers = []
...
class StyleEncoder(nn.Module):
    def __init__(
        self, dim_in=48, style_dim=48, num_speaker_domains=2, max_conv_dim=384
    ):
        super().__init__()
        blocks = []
...
class Discriminator(nn.Module):
    def __init__(
        self, dim_in=48, num_speaker_domains=2, max_conv_dim=384, repeat_num=4
    ):
        super().__init__()
        ...
        self.dis = Discriminator2d(
            dim_in=dim_in,
            num_speaker_domains=num_speaker_domains,
        ...
        self.aux_cls = Discriminator2d(
            dim_in=dim_in,
            num_speaker_domains=5,
            max_conv_dim=max_conv_dim,
            repeat_num=repeat_num,
        )
...
class Discriminator2d(nn.Module):
    def __init__(
        ...
        blocks += [nn.LeakyReLU(0.2)]
        blocks += [nn.AdaptiveAvgPool2d(1)]
        blocks += [nn.Conv2d(dim_out, num_speaker_domains, 1, 1, 0)]
        self.main = nn.Sequential(*blocks)

    def get_feature(self, x):
        out = self.main(x)
        out = out.view(out.size(0), -1)  # (batch, num_speaker_domains)
        return out
...
def build_model(args, F0_model, ASR_model):
    ...
    mapping_network = MappingNetwork(
        args.latent_dim,
        args.style_dim,
        args.num_speaker_domains,
        hidden_dim=args.max_conv_dim,
    )
    style_encoder = StyleEncoder(
        args.dim_in, args.style_dim, args.num_speaker_domains, args.max_conv_dim
    )
    discriminator = Discriminator(
        args.dim_in, args.num_speaker_domains, args.max_conv_dim, args.n_repeat
    )
    vocoder = load_model(args.vocoder_path)
    vocoder.remove_weight_norm()

    _, emotion_encoder = full_emo_bm()
    model_params = torch.load(args.emotion_encoder_path, map_location="cpu")[
        "model_ema"
    ]["coder"]
