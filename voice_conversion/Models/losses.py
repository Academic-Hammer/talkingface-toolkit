...
from dataset.Data.transforms import build_transforms
from voice_conversion.Utils.constants import *
...
def compute_g_loss(
    ...
    if use_feature_loss:
        ...
        with torch.no_grad():
            wav_real = [
                nets.vocoder.inference(mel_real[idx]) for idx in range(batch_size)
            ]
        ...
        if feature_loss_param.use_loudness_loss:
            ...
            if feature_loss_param.use_norm_loudness:
                loudness_real = loudness_real / torch.mean(
                    loudness_real, dim=-1, keepdim=True
                )
                loudness_fake = loudness_fake / torch.mean(
                    loudness_fake, dim=-1, keepdim=True
                )
            if feature_loss_param.use_delta_loudness:
                loudness_fake_change = torchaudio.functional.compute_deltas(
                    loudness_fake.unsqueeze(0)
                ).squeeze()
                with torch.no_grad():
                    loudness_real_change = torchaudio.functional.compute_deltas(
                        loudness_real.unsqueeze(0)
                    ).squeeze()
                loudness_loss = F.smooth_l1_loss(
                    loudness_real_change, loudness_fake_change
                )
            else:
        ...
        if (
            feature_loss_param.use_spectral_centroid_loss
            or feature_loss_param.use_spectral_bandwidth_loss
        ):
            to_spectral_centroid = get_spectral_centroid(feature_loss_param).to(
                wav_fake.device
            )
            ...
            if feature_loss_param.use_spectral_centroid_loss:
                ...
                freqs = (
                    torch.from_numpy(
                        librosa.fft_frequencies(
                            sr=feature_loss_param.sr, n_fft=MEL_PARAMS["n_fft"]
                        )
                    )
                    .type(sc_real.dtype)
                    .to(sc_real.device)
                ...
                sb_fake = torch.sum(
                    fake_spectrogram * fake_deviation**feature_loss_param.order,
                    axis=-2,
                    keepdims=True,
                ) ** (1.0 / feature_loss_param.order)
                sb_real = torch.sum(
                    real_spectrogram * real_deviation**feature_loss_param.order,
                    axis=-2,
                    keepdims=True,
                ) ** (1.0 / feature_loss_param.order)
        ...
        if feature_loss_param.use_spectral_kurtosis_loss:
            ...
            freqs = (
                ...
                .to(real_spectrogram.device)
                .unsqueeze(0)
                .unsqueeze(0)
            )

            fake_spread = torch.sqrt(
                torch.sum(
                    (freqs - sc_fake) ** 2 * fake_spectrogram, dim=-1, keepdim=True
                )
                / torch.sum(fake_spectrogram, dim=-1, keepdim=True)
            )
            real_spread = torch.sqrt(
                torch.sum(
                    (freqs - sc_real) ** 2 * real_spectrogram, dim=-1, keepdim=True
                )
                / torch.sum(real_spectrogram, dim=-1, keepdim=True)
            )

            fake_kurtosis = torch.sqrt(
                torch.sum(
                    (freqs - sc_fake) ** 4 * fake_spectrogram, dim=-1, keepdim=True
                )
                / (fake_spread**4 * torch.sum(fake_spectrogram, dim=-1, keepdim=True))
            )
            real_kurtosis = torch.sqrt(
                torch.sum(
                    (freqs - sc_real) ** 4 * real_spectrogram, dim=-1, keepdim=True
                )
                / (real_spread**4 * torch.sum(real_spectrogram, dim=-1, keepdim=True))
            )
        ...
        if feature_loss_param.use_deep_emotion_feature_loss:
            with torch.no_grad():
                deep_emo_feature_real = nets.emotion_encoder.encoder.get_shared_feature(
                    x_real
                )
                for l in list(nets.emotion_encoder.fc_o.modules())[1:-1]:
                    deep_emo_feature_real = l(deep_emo_feature_real)
            deep_emo_feature_fake = nets.emotion_encoder.encoder.get_shared_feature(
                x_fake
            )
            for l in list(nets.emotion_encoder.fc_o.modules())[1:-1]:
                deep_emo_feature_fake = l(deep_emo_feature_fake)
            deep_emo_feature_fake2 = nets.emotion_encoder.encoder.get_shared_feature(
                x_fake2
            )
            for l in list(nets.emotion_encoder.fc_o.modules())[1:-1]:
                deep_emo_feature_fake2 = l(deep_emo_feature_fake2)
            deep_emo_feature_loss = F.smooth_l1_loss(
                deep_emo_feature_fake, deep_emo_feature_real
            )
            deep_emo_feature_loss += F.smooth_l1_loss(
                deep_emo_feature_fake2, deep_emo_feature_real
            )
        else:
        ...
        if feature_loss_param.use_f0_related_loss:
            ...
            real_f0_local_mean = (F0_real[..., 1:] + F0_real[..., :-1]) / 2.0
            fake_f0_local_mean = (F0_fake[..., 1:] + F0_fake[..., :-1]) / 2.0

            relative_delta_f0_real = torch.nan_to_num(
                delta_f0_real / real_f0_local_mean
            )
            relative_delta_f0_fake = torch.nan_to_num(
                delta_f0_fake / fake_f0_local_mean
            )

            f0_related_loss += F.smooth_l1_loss(
                relative_delta_f0_real, relative_delta_f0_fake
            )

            # Binarize
            delta_f0_real_up_slope = torch.clamp(
                F.relu(delta_f0_real), min=0.0, max=1.0
            )
            delta_f0_real_down_slope = torch.clamp(
                F.relu(delta_f0_real * -1), min=0.0, max=1.0
            )

            delta_f0_fake_up_slope = torch.clamp(
                F.relu(delta_f0_fake), min=0.0, max=1.0
            )
            delta_f0_fake_down_slope = torch.clamp(
                F.relu(delta_f0_fake * -1), min=0.0, max=1.0
            )

            f0_related_loss += F.smooth_l1_loss(
                delta_f0_real_up_slope, delta_f0_fake_up_slope
            )
            f0_related_loss += F.smooth_l1_loss(
                delta_f0_real_down_slope, delta_f0_fake_down_slope
            )
    ...
    loss_norm = (
        (torch.nn.ReLU()(torch.abs(x_fake_norm - x_real_norm) - args.norm_bias)) ** 2
    ).mean()
    ...
    loss_cyc += F.smooth_l1_loss(
        nets.generator.get_content_representation(x_fake.detach()),
        nets.generator.get_content_representation(x_real.detach()).detach(),
    )
    loss_cyc += F.smooth_l1_loss(
        nets.generator.get_content_representation(x_fake2.detach()),
        nets.generator.get_content_representation(x_real.detach()).detach(),
    )
