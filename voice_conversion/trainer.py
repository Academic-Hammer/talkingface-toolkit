...
class Trainer(object):
    ...
    def save_checkpoint(self, checkpoint_path):
        ...
        if self.model_ema is not None:
            state_dict["model_ema"] = {
                key: self.model_ema[key].state_dict() for key in self.model_ema
            }
    ...
    def _load(self, states, model, force_load=True):
        ...
        for key, val in states.items():
            try:
                ...
                self.logger.info(f"not exist :{key}")
                print("not exist ", key)
    ...
    def _train_epoch(self):
        ...
        use_feature_loss = (
            self.epochs >= self.args.g_loss["feature_loss"]["feature_loss_epoch"]
        )

        for train_steps_per_epoch, batch in enumerate(
            tqdm(self.train_dataloader, desc="[train]"), 1
        ):
            ### load data
            batch = [b.to(self.device) for b in batch]
            x_real, y_org, sp_org, x_ref, x_ref2, y_trg, z_trg, z_trg2 = (
                batch["mels"],
                batch["labels"],
                batch["sp_labels"],
                batch["ref_mels"],
            ...
            self.moving_average(
                self.model.generator, self.model_ema.generator, beta=0.999
            )
            self.moving_average(
                self.model.mapping_network, self.model_ema.mapping_network, beta=0.999
            )
            self.moving_average(
                self.model.style_encoder, self.model_ema.style_encoder, beta=0.999
            )
            self.optimizer.scheduler()

            for key in d_losses_latent:
                train_losses[f"train/{key}"].append(d_losses_latent[key])
            for key in g_losses_latent:
                train_losses[f"train/{key}"].append(g_losses_latent[key])
            for key in g_losses_ref:
                train_losses[f"train_ref/{key}"].append(g_losses_ref[key])
    ...
    def _eval_epoch(self):
        ...
        for eval_steps_per_epoch, batch in enumerate(
            ...
            for key in d_losses_latent:
                eval_losses[f"eval/{key}"].append(d_losses_latent[key])
            for key in g_losses_latent:
                eval_losses[f"eval/{key}"].append(g_losses_latent[key])
    ...
    def _sample_write_epoch(self, sample_write_params, device="cpu"):
        if (
            self.save_samples
            and self.gen_dataloader is not None
            and self.epochs % 5 == 0
        ):
            _ = [self.model[k].eval() for k in self.model]
            if (
                "Emotional Speech Dataset (ESD)"
                in sample_write_params["real_sample_path"]
            ):
            ...
            for gen_steps_per_epoch, batch in enumerate(
                tqdm(self.gen_dataloader, desc="[generate sample]"), 1
            ):
                ### load data
                batch = [b.to(self.device) for b in batch]
                x_ref, y_trg, _, _, _, _, _, _ = (
                    batch["mels"],
                    batch["labels"],
                    batch["sp_labels"],
                    batch["ref_mels"],
